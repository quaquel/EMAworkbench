...
def pick_csamples(clusters, distRow):
    dMatrix = squareform(distRow)
    c = clusters==2
    print c
    return dMatrix
...
if __name__ == '__main__':
    ...
    experiments, results = load_results('datasets/PatternSet_Basics.cPickle')
    data = results['outcome']
    tester = np.array([2,4,8])
    tester2 = np.array([2,4,8,2,4,6,2])
    print tester.__class__
    print tester2.__class__
    print tester2
    k = tester2[tester2>4]
    print k
    k[1] = 4
    ...
    print squareform(tester, force='tomatrix')
