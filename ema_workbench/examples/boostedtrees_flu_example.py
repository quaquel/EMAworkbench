...
def plot_factormap(x1, x2, ax, bdt, nominal):
    ...
    ax.contourf(xx, yy, Z, cmap=plt.cm.Paired, alpha=.5)  # @UndefinedVariable
...
def plot_diag(x1, ax):
    ...
    for i in (0, 1):
        idx = y == i
        ax.hist(x[idx, x1], range=(x_min, x_max), alpha=.5)


# load data
experiments, outcomes = load_results(
    './data/1000 flu cases with policies.tar.gz')
...
for i, row in enumerate(axes):
    for j, ax in enumerate(row):
        if i > j:
            plot_factormap(sorted_indices[j], sorted_indices[i], ax, bdt,
                           nominal)
        elif i == j:
...
handles = [CircleCollection([10], color=sns.color_palette()[0]),
