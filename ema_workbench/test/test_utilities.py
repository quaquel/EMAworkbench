...
def load_eng_trans_data():
    ...
    with zipfile.ZipFile(fn) as z:
        experiments = io.StringIO(z.read('experiments.csv'))
        ...
        data = io.StringIO.StringIO(z.read('total capacity installed.csv'))
        outcomes['total capacity installed'] = np.loadtxt(data, delimiter=',')
         
        data = io.StringIO(z.read('TIME.csv'))
        outcomes[TIME] = np.loadtxt(data, delimiter=',')
      
        data = io.StringIO(z.read('total fraction new technologies.csv'))
