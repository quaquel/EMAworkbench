...
def make_data_structure(clusters, distRow, runLogs):
    ...
    for i in range(1, nr_clusters+1):
        ...
        drow_indices = np.zeros(
            (indices.shape[0]**2-indices.shape[0])/2, dtype=int)
        s = 0
...
if __name__ == '__main__':
    ...
    distance, liste, obekler, kosulog, zet = cluster(
        results, 'outcome', cMethod='distance', cValue=1, groupPlot=True, plotDendrogram=False)
